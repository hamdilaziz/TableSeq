...
def tree_convert_html(node, convert_cell=False, parent=None):
    ...
    if node.tag == 'td':
        if convert_cell:
            ...
            cell = []
        colspan = node.attrib.get('colspan', '1')
        colspan = int(colspan) if colspan.isdigit() else 1
        rowspan = node.attrib.get('rowspan', '1')
...
def eval_TEDS(gt_raw, pred_raw, structure_only=False):
    # remove location tokens 
    gt_raw = re.sub(r"<x_\d+>|<y_\d+>|<table>|</table>", "", gt_raw)
    pred_raw = re.sub(r"<x_\d+>|<y_\d+>|<table>|</table>", "", pred_raw)
    ...
    return similarity_eval_html(html.fromstring(pred_string), html.fromstring(gt_string),structure_only=structure_only)