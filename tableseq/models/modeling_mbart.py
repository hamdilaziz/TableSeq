...
class MBartAttention(nn.Module):
    ...
    def forward(
        ...
        key_bias: Optional[torch.Tensor] = None,   # <-- key-only prior (B, Tk) or (B,1,1,Tk)
        ...
        if (
            ...
            # reuse cached cross-attn K,V
            key_states = past_key_value[0]
            value_states = past_key_value[1]
        elif is_cross_attention:
            # fresh cross-attn K,V
            key_states = self._shape(self.k_proj(key_value_states), -1, bsz)
            value_states = self._shape(self.v_proj(key_value_states), -1, bsz)
        elif past_key_value is not None:
            # decoder self-attn with cache
            key_states = self._shape(self.k_proj(hidden_states), -1, bsz)
            value_states = self._shape(self.v_proj(hidden_states), -1, bsz)
            key_states = torch.cat([past_key_value[0], key_states], dim=2)
            value_states = torch.cat([past_key_value[1], value_states], dim=2)
        else:
            # decoder self-attn no cache
            key_states = self._shape(self.k_proj(hidden_states), -1, bsz)
            value_states = self._shape(self.v_proj(hidden_states), -1, bsz)
        ...
        # raw scores [B*H, T_q, T_k]
        attn_weights = torch.bmm(query_states, key_states.transpose(1, 2))

        if attn_weights.size() != (bsz * self.num_heads, tgt_len, src_len):
            raise ValueError(
                f"Attention weights should be {(bsz * self.num_heads, tgt_len, src_len)}, "
                f"but is {attn_weights.size()}"
            )

        # ---- Add masks & key_bias in 4D ----
        attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len)

        if attention_mask is not None:
            if attention_mask.size() != (bsz, 1, tgt_len, src_len):
                raise ValueError(
                    f"Attention mask should be {(bsz, 1, tgt_len, src_len)}, but is {attention_mask.size()}"
                )
            attn_weights = attn_weights + attention_mask  # additive (0 or -inf)

        if key_bias is not None:
            # Accept [B, Tk] or [B,1,1,Tk]
            if key_bias.dim() == 2:
                key_bias = key_bias[:, None, None, :]  # -> [B,1,1,Tk]
            elif key_bias.dim() != 4:
                raise ValueError(f"key_bias must be [B,Tk] or [B,1,1,Tk], got {tuple(key_bias.shape)}")
            if key_bias.size(-1) != src_len:
                raise ValueError(f"key_bias last dim {key_bias.size(-1)} != src_len {src_len}")
            key_bias = key_bias.to(dtype=attn_weights.dtype, device=attn_weights.device)
            attn_weights = attn_weights + key_bias  # broadcast over heads & queries
            del key_bias

        # back to 3D for softmax/dropout/bmm
        attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
        ...
        if layer_head_mask is not None:
            if layer_head_mask.size() != (self.num_heads,):
                raise ValueError(
                    f"Head mask for a single layer should be {(self.num_heads,)}, but is {layer_head_mask.size()}"
                )
...
class MBartDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        encoder_key_bias: Optional[torch.Tensor] = None,   # <-- NEW: [B, Tk] or [B,1,1,Tk]
    ) -> Tuple[torch.Tensor, ...]:
        """
        hidden_states: (B, T_q, D)
        attention_mask: (B, 1, T_q, T_q) for decoder self-attn
        encoder_hidden_states: (B, T_k, D)
        encoder_attention_mask: (B, 1, T_q, T_k)
        """
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states, cross_attn_weights, cross_attn_present_key_value = self.encoder_attn(
                ...
                output_attentions=output_attentions,
                key_bias=encoder_key_bias,   # <-- PASS THE PRIOR
            )

            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            hidden_states = residual + hidden_states

            # append cross-attn cache
...
class MBartDecoder(MBartPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        encoder_key_bias: Optional[torch.Tensor] = None,
